(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".minus", function () {
            ...
            var min = parseInt($input.attr("min")); // Lấy giá trị min từ thuộc tính min của input
            count = count < min ? min : count; // Đảm bảo giá trị không nhỏ hơn min
        ...
        $(document).on("click", ".plus", function () {
            var $input = $(this).siblings("input.quantity");
            var count = parseInt($input.val()) + 1;
            var max = parseInt($input.attr("max")); // Lấy giá trị max từ thuộc tính max của input
            count = count > max ? max : count; // Đảm bảo giá trị không lớn hơn max
            $input.val(count).change();
            return false;
        });
